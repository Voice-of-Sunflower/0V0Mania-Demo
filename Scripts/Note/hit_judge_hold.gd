...
enum hold_states { INIT, IS_HEAD, IS_HOLDING, IS_NOT_HOLD, IS_BREAK, IS_END}
...
## ATTENTION 才发现 is_action_pressed() 只能截取到按的那一帧，部分判定还是需要放到_process()中
func _unhandled_input(event):
	...
	if event.is_action_pressed("Key_" + str(note_track)):
...
func _process(delta):
	...
	if is_auto_play:
		...
		## 在hold结尾处于松开按键的状态下判定为miss
			...
			match current_state:
				## 过短的音符不能进入 IS_NOT_HOLD 状态，需要加入 IS_HEAD 的判定
				hold_states.IS_HEAD:
					after_note_end(MISS)
				hold_states.IS_NOT_HOLD:
					after_note_end(MISS)
				hold_states.IS_HOLDING:
					after_note_end(PERFECT)
				hold_states.IS_BREAK:
					after_note_end(GOOD)

func after_note_end(rate: int):
	current_state = hold_states.IS_END
	GlobalVariable.hit_sound.play()
	note_judger.judge_rate_hold(rate)
	IngameVariable.current_track_note_index[note_track - 1] = current_hold_index + 1
...
func input_check_improve_press_part() -> void:
	...
	match current_state:
		...
		## 这个版本的_input()无法识别到此状态，移动到_process()中
		#hold_states.IS_HOLDING:
