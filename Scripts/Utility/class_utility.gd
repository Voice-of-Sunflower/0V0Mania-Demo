...
## 重命名文件
static func rename_file(file_path: String, target_name: String):
	var origin_file_name: String = file_path.get_file()
	var origin_file_path: String = file_path.get_base_dir()
	DirAccess.open(origin_file_path).rename(origin_file_name, target_name)

## 重命名目录
static func rename_dir(file_path: String, target_path: String):
	var origin_file_path: String = file_path.get_base_dir()
	DirAccess.open(origin_file_path).rename(file_path, target_path)

static func json_save(file_path: String, data):
	var file = FileAccess.open(file_path, FileAccess.WRITE)
	var convert_data = JSON.stringify(data)
