...
@export var osu_beatmap_converter: Node
...
func _on_midi_file_dialog_file_selected(source_path: String):
	if source_path.get_extension() == "osu":
		var target_path = beatmap_path.path_join("beatmap.osu")
		DirAccess.copy_absolute(source_path, target_path)
		beatmap_file_edit.text = target_path
	
	elif source_path.get_extension() == "mid":
...
func create_level_dictory():
	...
	var file_ext: String = beatmap_file_edit.text.get_extension()
	
	if file_ext == "osu":
		osu_beatmap_converter.osu_file_converter(beatmap_file_edit.text)
		var note_array: Array = osu_beatmap_converter.sort_note_group_time()
		var hold_array: Array = osu_beatmap_converter.sort_hold_group_time()
		
		osu_beatmap_converter.save_beatmap_json(note_array, false, beatmap_path.path_join(level_name_edit.text))
		osu_beatmap_converter.save_beatmap_json(hold_array, true, beatmap_path.path_join(level_name_edit.text))
	elif file_ext == "mid" :
