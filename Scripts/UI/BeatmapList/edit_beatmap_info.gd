...
func check_and_save_info():
	...
	if is_edit:
		...
		DirAccess.open(beatmaps_path).rename(origin_path, music_name)
		is_edit = false
	...
	var file = FileAccess.open(beatmap_info_file_path, FileAccess.WRITE)
	var convert_data = JSON.stringify(beatmap_info)
